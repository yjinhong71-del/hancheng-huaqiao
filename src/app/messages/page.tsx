      ...
      {conversations.map(c => (
        <button
          ...
          className={`w-full text-left px-4 py-3 flex items-center gap-3 hover:bg-black/[0.02] transition-colors ${
            activeChat === c.partner_id ? 'bg-black/[0.04]' : ''
          }`}
        >
          <div className="w-10 h-10 rounded-full bg-black/[0.04] flex items-center justify-center shrink-0">
            {c.partner_photo ? (
              <img src={c.partner_photo} className="w-full h-full rounded-full object-cover" alt="" />
            ) : (
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-neutral-500 truncate mt-0.5">
              {c.last_sender_id === userPersonId ? '你: ' : ''}
              {c.last_message || '开始聊天...'}
            </p>
          </div>
        </button>
  ...
  return (
    <>
      ...
      <button
        ...
        onClick={onSend}
        disabled={!input.trim()}
        className="p-2.5 sm:p-3 bg-blue-500 text-white rounded-full hover:bg-blue-600 transition-colors active:scale-[0.95] disabled:opacity-40 disabled:cursor-not-allowed shrink-0"
      >
        <Send size={16} />
      </button>
    </>
...
export default function MessagesPage() {
  ...
  useEffect(() => {
    fetchUser().then(() => fetchConversations());
            ...
            setConversations(prev => [
              {
                ...
                partner_photo: person.photo_url || '',
                last_message: '',
                last_time: '',
                last_sender_id: '',
                unread: 0,
        ...
        try {
          ...
          if (data.type === 'new_message') {
            ...
            if (
              ...
              if (activeChat !== '__anonymous__')
                fetch(`/api/messages/read?with=${activeChat}`, { method: 'PUT' });
            }
            fetchConversations();
          }
        } catch {}
          ...
          <div className="hidden md:flex h-full">
            <div className="w-[280px] shrink-0 border-r border-black/[0.06] flex flex-col">
              <div className="p-4 border-b border-black/[0.04]">
                <h3 className="text-sm font-semibold text-neutral-700">对话</h3>
              </div>
              <ConversationList
                  ...
                  <div className="flex-1 overflow-y-auto p-4 space-y-3">
                    {messages.map(m => (
                      <div
                        key={m.id}
                        className={`flex ${
                          m.sender_id === user?.personId ? 'justify-end' : 'justify-start'
                        }`}
                      >
                        <div
                          className={`max-w-[75%] px-4 py-2.5 rounded-2xl text-sm ${
                            m.sender_id === user?.personId
                              ? 'bg-blue-500 text-white rounded-br-md'
                              : 'bg-black/[0.05] text-black rounded-bl-md'
                          }`}
                        >
                          {m.content}
                        </div>
          ...
          <div className="md:hidden h-full flex flex-col">
            {mobileView === 'list' ? (
              ...
              <div className="flex flex-col h-full">
                <div className="p-3 border-b border-black/[0.04] flex items-center gap-3 shrink-0">
                  <button
                    onClick={() => {
                      setMobileView('list');
                      setActiveChat(null);
